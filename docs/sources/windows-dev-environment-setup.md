...
Handoff doc for Asaf's project-instructions plugin. This describes a complete,
...
> **Portability note for the plugin:** paths like `/home/<user>` and the Windows
> username appear below. In the source these were hardcoded (`/home/nird`,
> `C:\Users\nirda`). For a reusable plugin, substitute `$HOME` on the Linux side
> and resolve the Windows user/profile at runtime (PowerShell
> `[Environment]::GetFolderPath(...)`). Spots that MUST be generalized are flagged
> ⚠️.
...
⚠️ The shipped script hardcodes `/home/nird/.local/bin/herdr` and
`/usr/bin/ghostty`; generalize the herdr path to `$HOME/.local/bin/herdr`.
...
```
⚠️ `-d Ubuntu` = distro name (`wsl -l` to list); `/home/<user>/...` is Linux-side.
